...
{
    expr->data = (mp_limb_t*)flint_malloc(sizeof(ulong));
    expr->data[0] = 0;
    expr->alloc = 1;
}
...
fexpr_fit_size(fexpr_t expr, slong size)
{
    if (expr->alloc < size)
    {
        ...
        expr->data = (mp_limb_t*)flint_realloc(expr->data, size * sizeof(ulong));
        expr->alloc = size;
    }
}
...
fexpr_vec_init(fexpr_vec_t vec, slong len)
{
    ...
    {
        slong i;
        vec->entries = (fexpr_struct*)flint_malloc(sizeof(fexpr_struct) * len);
        for (i = 0; i < len; i++)
